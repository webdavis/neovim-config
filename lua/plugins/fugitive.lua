return {
  ...
  config = function()
    ...
    vim.keymap.set("n", "<C-g>cc", "<cmd>Git commit<cr>", { noremap = true, desc = 'commit everything in staging area' })
    vim.keymap.set("n", "<C-g>cv", "<cmd>Git commit --verbose<cr>",
    ...
    vim.keymap.set("n", "<C-g>cn", "<cmd>Git commit --amend --no-edit<cr>",
      { noremap = true, desc = 'amend commit without editing the message' })

    vim.keymap.set('n', '<C-g>pr', function() vim.cmd.Git('pull --rebase') end,
      { noremap = true, desc = 'pull and rebase' })
    vim.keymap.set('n', '<C-g>pp', function() vim.cmd.Git('push') end, { noremap = true, desc = 'push' })
    vim.keymap.set('n', '<C-g>pf', function() vim.cmd.Git('push --force') end,
      { noremap = true, desc = 'force push' })
    vim.keymap.set('n', '<C-g>pu', function()
      ...
      if current_branch then
        ...
        local confirm_push
        vim.ui.input({ prompt = 'Are you sure you want to push to the branch, ' .. current_branch .. ' ? y/n: ' },
          function(input)
            confirm_push = input
          end)
    ...
    vim.api.nvim_set_keymap('n', '<C-g>of', '<cmd>GBrowse<cr>',
      { noremap = true, silent = true, desc = 'browse file in browser' })
    vim.api.nvim_set_keymap('n', '<C-g>ol', '<cmd>.GBrowse<cr>',
      { noremap = true, silent = true, desc = 'browse current line in browser' })
    vim.keymap.set('n', '<C-g>or', function() vim.fn.system({ 'gh', 'browse' }) end,
      { noremap = true, silent = false, desc = 'browse repo in browser' })
  end,
}
