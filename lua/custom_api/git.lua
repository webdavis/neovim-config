...
local function branch(opts)
  ...
  local _, branches = run_shell_command("git branch")
...
local function latest_commit(opts)
  ...
  local hash_exit, hash = run_shell_command("git rev-parse --short HEAD")
  if hash_exit ~= 0 then
    return nil,
      nil,
      nil,
  ...
  local message_exit, message = run_shell_command("git log -1 --pretty=%B")
...
local function url(opts)
  ...
  local command = string.format("git config --get remote.%s.url", remote)
